...
(defn endpoint
  "Returns the OpenPlanner events endpoint URL from config."
  [config]
  (str (:openplanner-base-url config) "/v1/events"))

(defn make-state
  "Returns a fresh OpenPlanner queue state object."
  [_config]
  #js {"queue" #js []
...
(defn queue-event!
  "Appends event to the state queue, trimming to max-persisted length."
  [^js state event max-persisted]
  (when event
...
(defn ^:async flush!
  "Flushes queued events to the OpenPlanner events endpoint."
  [^js state config]
...
(defn ^:async query-semantic-similar
  "Queries OpenPlanner for messages semantically similar to text, returning high-confidence hits."
  [text k config]
...
(defn ^:async run-semantic-scan!
  "Checks pending message ids for existing embeddings and clears them from the pending set."
  [^js state config]
...
(defn start-timers!
  "Starts the flush and semantic-scan intervals on state."
  [^js state config]
...
(defn stop-timers!
  "Stops any running timers on state."
  [^js state]
