(ns bitch-tracker.plugin
  (:require [clojure.string :as str]
            ...
            [bitch-tracker.plugin.socket-client :as socket]))
...
(defn- log! [level meta & xs]
  (let [name (meta-field meta "name" c/plugin-name)
...
(defn- toast! [message type]
  (when-let [show-toast (u/jget (bd-api) "UI" "showToast")]
    (.call show-toast (u/jget (bd-api) "UI") message #js {:type type})))

(defn- bd-data-load [plugin key]
  (u/jcall (u/jget (bd-api) "Data") "load" plugin key))

(defn- bd-data-save! [plugin key value]
  (u/jcall (u/jget (bd-api) "Data") "save" plugin key value))
...
(defn- connect-to-bot! [state meta]
...
(defn settings-panel [meta state]
  (when-let [document (global-get "document")]
    (let [root (.createElement document "div")
      ...
      (set! (.-textContent title) (meta-field meta "name" c/plugin-name))
...
(defn ^:async start-plugin! [meta state]
  ...
  (log! "info" meta "started" (str "v" (meta-field meta "version" c/plugin-version)))
  (toast! (str (meta-field meta "name" c/plugin-name) " started") "success")
  state)
...
(defn stop-plugin! [meta state]
  ...
  (toast! (str (meta-field meta "name" c/plugin-name) " stopped") "info")
  state)
